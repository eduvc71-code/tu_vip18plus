...
export function getBotConfig() {
  ...
  let rawUsername = process.env.BOT_USERNAME || storedUsername || 'Danii_OficialVIP_bot';
  if (/ruti|flavia|catalogovip|iam_danii/i.test(rawUsername)) {
    rawUsername = 'Danii_OficialVIP_bot';
  }
...
export async function sendClientWelcome(chatId: string | number, firstName: string = 'Invitado/a') {
  ...
  const cleanUsername = username || process.env.BOT_USERNAME || 'Danii_OficialVIP_bot';
  const inviteLink = `https://t.me/${cleanUsername}?start=inv_vip`;
